...
export default function Customer () {
  ...
  async function getCustomer(customerId){
    const data = axios.get(`http://10.202.36.105:8000/api/customer/${customerId}`,{
      headers : {
  ...
  function saveSalesHistory(salesHistory){
    axios.post(`http://10.202.36.105:8000/api/sales-history`, salesHistory, {
      headers : {
          Authorization : `Bearer ${localStorage.getItem('access_token')}`
      }
    }).then(({data})=>{
                ...
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    ...
                    <Stack direction="row" spacing={1}>
                      ...
                      <Button variant="contained" onClick={()=>(alert())}>신규고객등록</Button>
